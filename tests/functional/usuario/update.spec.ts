...
test.group('Usuario update', (group) => {
  ...
  test('atualizar todos os parâmetros de um usuário', async ({ client, assert }) => {
    ...
    const response = await client.patch('/usuarios').json(novo).loginAs(antigo)
  ...
  test('atualizar cada parâmetros de um usuário individualmente', async ({ client, assert }) => {
    ...
    let response = await client.patch('/usuarios').json({ nome: novo.nome }).loginAs(antigo)
    ...
    response = await client.patch('/usuarios').json({ telefone: novo.telefone }).loginAs(antigo)
    ...
    response = await client.patch('/usuarios').json({ email: novo.email }).loginAs(antigo)
    persistido = await Usuario.findOrFail(id)
    response.assertStatus(200)
    assert.equal(response.body()['email'], novo.email)
    assert.equal(persistido.nome, novo.nome)
